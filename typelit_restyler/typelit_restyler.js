...
(function() {
    ...
    function waitForElement(elemFetchHandle, actionHandle, interval) {
        let intervalId = setInterval(() => {
            const elem = elemFetchHandle();
            if (!elem) {
                console.log(`Waiting for element to load. Checking again in ${interval}ms.`);
                return;
            }
    ...
    function settingsButtonAction() {
        ...
        waitForElement(() => {
            ...
            // this is necessary because the settings button gets replaced when the settings close.
